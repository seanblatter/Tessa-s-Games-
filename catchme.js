(function () {
    ...
    const state = {
        ...
        mapStyle: localStorage.getItem(MAP_STYLE_KEY) || 'streets', totalDistanceKm: 0, lastAccepted: null, runStartTs: null, runTimer: null
    };
    const ui = {};
    ...
    const formatDuration = (ms) => { const total = Math.max(0, Math.floor(ms/1000)); const m = String(Math.floor(total/60)).padStart(2,'0'); const s = String(total%60).padStart(2,'0'); return `${m}:${s}`; };
    function refreshRunTime(final=false){ if(!ui.runtime) return; if(!state.runStartTs){ ui.runtime.textContent='Run time: 00:00'; return;} const elapsed=Date.now()-state.runStartTs; ui.runtime.textContent=`Run time${final?' (final)':''}: ${formatDuration(elapsed)}`;} 
    ...
    async function onPosition(pos) {
        ...
        if (!candidate) {
            refreshRunTime();
            setMessage(`Tracking run · ${state.runPath.length} pts · ${toDist(state.totalDistanceKm)} · ±${sample.accuracy.toFixed(2)}m`);
            return;
        }
        ...
        updateProgress();
        refreshRunTime(true);
        setMessage(`Loop complete and saved: ${toArea(turf.area(clipped))}. Duration ${formatDuration(Date.now()-state.runStartTs)}.`);
        stopRun(false, true);
    }
    ...
    function startRun() {
        ...
        state.runTimer = setInterval(() => refreshRunTime(), 1000);
        refreshRunTime();
    ...
    function stopRun(showMsg = true, preserveFinal=false) {
        ...
        if (!preserveFinal) refreshRunTime(true);
        state.runPath = []; state.runSamples = []; state.totalDistanceKm = 0; state.lastAccepted = null; state.runStartTs = null;
    ...
    async function bootstrap() {
        ...
        ui.centerBtn.onclick = () => state.map.setView(CHICAGO_CENTER, 12);
        ui.drawerToggle.onclick = () => {
